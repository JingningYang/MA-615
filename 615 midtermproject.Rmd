...
## Read the "World Value" data {.smaller}
...
## Clean data: Convert selected survey questions from character to numeric: {.smaller}
```{r, echo=FALSE}
...
## Preparation for factor analysis    {.smaller}
...
## Scree plot by using another way:  {.smaller}
```{r, echo=FALSE}
...
## Orthogonal rotation(varimaax): {.smaller}
...
## Obersevation: {.smaller .flexbox .vcenter}
...
## Ploting PCA  {.smaller}
Relationship between varaibles and our components:
...
#dev.new(width=10, height=10)
#biplot(princomp(newdata[x])) #plot pincipal components 1 vs 2

```

## Ploting PCA  {.smaller}
Relationship between compononet 1 vs 2:  kind of linearity.
```{r, echo=FALSE, message=FALSE}
library(ggfortify)
...
## Ploting PCA  {.smaller}
Relationship between component by using cluster:
```{r, echo=FALSE}
library(cluster)
library(fpc)
